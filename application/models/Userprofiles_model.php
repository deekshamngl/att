...
public function get_last_ten_entries()
      ...
      return $res;
	       ...
	       public function getEmployeesData_testing()
      {
         $orgid=$_SESSION['orgid'];
    ...
    FROM EmployeeMaster where`OrganizationId` = 10 and Is_Delete != 2) as registeredusers from licence_ubiattendance where `OrganizationId`=$orgid");
         ...
         foreach ($query->result() as $row)
        {
          ...
          $data['photo']='<i class="pop" data-toggle="modal" data-target="#imagemodal"><img src="'.IMGURL3."avatars/male.png".'"    style="width:50px!important;height:50px!important;border-radius:50%"  /></i>';
            ...
            $data['code']=$row->ecode;
            $data['username']=  "dummy";//decode5t(getUserName($row->id));
          ...
          else
          {
            ...
            $data['contact']=  "dummy";//decode5t($row->PersonalNo);
    ...
    FROM EmployeeMaster where`OrganizationId` = 10 and Is_Delete != 2) as registeredusers from licence_ubiattendance where `OrganizationId`=$orgid");
        ...
        {
          if($row->archive==1)
            $userstts='<i class=" status fa fa-thumbs-down" style="font-size:16px; color:red" data-id="'.$row->id.'" data-sts="'.$row->archive.'" data-ena="'.$row->FirstName.'" title="Make user inactive" ></i>';
          else
            $userstts='<i class=" status fa fa-thumbs-up" style="font-size:16px; color:green" data-toggle="modal" data-id="'.$row->id.'" data-ena="'.$row->FirstName.'"  title="Click to make user Active" ></i>';
              $pass = decode5t($row->upa);
    ...
    { 
      ...
      $zone=getTimeZone($orgid);
        ...
        $username =  encode5t(isset($_REQUEST['fname'])?encode5t($_REQUEST['fname']):0);
        // var_dump($password);
   ...
   //  echo encode5t($_REQUEST['PersonalNo']);
    ...
    {
      $sql = "INSERT INTO EmployeeMaster (FirstName,EmployeeCode,DOB,DOJ,DOC,Gender,Nationality,MaritalStatus,Religion,BloodGroup,Department,Designation,Shift,archive,CurrentCountry,HomeCity,HomeAddress,PersonalNo,OrganizationId,CreatedDate,area_assigned,CompanyEmail) VALUES 
        (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
      ...
      // die();
        ...
        // die;
          if($result >0){
            $sql = "INSERT INTO UserMaster (EmployeeId,Password,Username,username_mobile,appSuperviserSts,Archive,OrganizationId) VALUES ($emp_id,'$password','$username','$cont','$sts1','$sts',$orgid)";
            $this->db->query($sql);
            // var_dump($this->db->last_query());
            // die;
            $result =  $this->db->affected_rows();
    ...
    //////////---------check user slimit--close
           ...
           $id =$_SESSION['id'];
           $module = "Employees";
           $actionperformed = " <b>".$fname."</b> has been added from <b>Add Employee </b>.";
           $activityby = 1;
      ...
      $rer = "";
        ...
        $sql2 = "INSERT INTO Temp_user_csv(FirstName,LastName,EmployeeCode,Department,Designation,Shift,PersonalNo,OrganizationId,CreatedDate,email,Archive,createdBy,remark,country) VALUES 
         ...
         if($data[3] != '')
          ...
          if($data[7] != '')
            {
            $query2 = $this->db->query("select Id FROM EmployeeMaster WHERE EmployeeCode=? AND OrganizationId = ? and Is_Delete!= 2 ",array($data[$ecode],$orgid));
          $query2->num_rows();
  ...
  public function deleteallemp_permanent()
     {
      ...
      $zone=getTimeZone($orgid);
       ...
       if($this->db->affected_rows()>0)
       {
         ...
         $deletequery = $this->db->query("Select OrganizationId ,AttendanceDate, EntryImage ,ExitImage from ArchiveAttendanceMaster where EmployeeId IN ($favorite1) ");
         {
