<?php
class Orginfo_model extends CI_Model {
...
*/
	...
	public function getstorepath(){
			...
			$res1=array();
			$id = $_SESSION['orgid'];
			$query = $this->db->query("SELECT * FROM PlayStore ");
			foreach ($query->result_array() as $row)
			{
		...
		public function updatePath(){
			//$id = $_SESSION['orgid'];
			$googlepath = $_REQUEST['googlepath'];
			$applepath = $_REQUEST['applepath'];
			
			$query = $this->db->query("update PlayStore set googlepath='$googlepath', applepath='$applepath'");
			if($query>0){
				//$_SESSION['name']=$pname;
				echo 1;
			}else{
		...
		public function trial(){
			...
			$sql = "update licence_ubiattendance set start_date='$start_date', end_date='$till_date' ,extended=$extended_data , status = 0 where OrganizationId=$org_id";
		...
		public function buy(){
			...
			$sql = "update  licence_ubiattendance set start_date='$start_date', end_date='$end_date'  , status = 1 where OrganizationId=$org_id";
			$query = $this->db->query($sql);
