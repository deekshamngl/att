	...
	public function getMonthlyAverageSummary($type)
        { 
				...
				$rangedate = 1;
					...
					$list['admin']=getAdminName($orgid);
					$list['email']=getAdminEmail($orgid);
					$q1=$this->db->query("Select if(E.LastName!='NULL',concat(E.FirstName, ' ' ,E.LastName),E.FirstName) as name , D.Name as desg ,E.Id,Shift from EmployeeMaster E , DesignationMaster D where E.Designation = D.id and E.OrganizationId = $orgid and E.archive = 1 and E.Is_Delete = 0 order by E.FirstName"  );
					foreach($q1->result() as $row1)
					{			
					$data=array();
					$empid=$row1->Id;
		...
		$timeFormat4 	= 	sprintf('%02d:%02d',$hours, $mins);
			...
			foreach($result as $r)
			{
				...
				if(count($r['report'])>0)
				{
					...
					['.$date.']
					<div style="margin-top:5%;">
					<table style="border-collapse: collapse;" border width="100%"><tr style="color:#fa6804"><th>S.No</th><th style="width:25%">Employees</th><th
					    style="width:15%;">Designation</th><th 
					    style="width:15%;">Present Days</th><th 
					    style="width:15%;">Absent Days</th><th 
					    style="width:15%;">Late Marks</th><th 
					    style="width:15%;">Total Logged Hours</th><th 
					    style="width:15%;">Total Late Coming</th><th 
					    style="width:15%;">Total Early Leaving</th><th 
					    style="width:15%;">Total Time <br/>Off</th>
