...
//	return false;
	/////////////********************************
	 ...
	 $query = $this->db->query("INSERT INTO `payments_invoice`( `txnid`, `OrganizationId`, `name`, `email`, `payment_amount`, `payment_status`, `createDate`,`payment_method`, `tax`, `discount`, `currency`, `country`, `state`, `city`, `street`, `contact`, `zip`, `indivisual_name`, `gstin`, `remark`,  `narration`, `action`, `Addon_BulkAttn`, `Addon_LocationTracking`, `Addon_VisitPunch`, `Addon_GeoFence`, `Addon_Payroll`, `Addon_TimeOff`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",array($txnid,$org_id,$Company,$email,$amount,'SUCCESS',$today,$paymentmthd,$tax,$discount,$currency,$country,$state,$city,$street,$contact,$zip,$ind_name,$gstin,'',$narration,'BUY',$bulk_att,$loc_trace,$visit_punch,$geo_fence,$payroll,$timeoff));
