...
class Att_services_zone extends CI_Controller{
	function __construct()
    {
        parent::__construct();
		$this->load->model('Att_services_zone_model');
    }
	
	public function getCountries(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getCountries();
	}
	public function getAttendancees(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getAttendancees();
	}
	public function getAreaStatus(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getAreaStatus();
	}
	
	public function getOrganization($id){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getOrganization($id);
	}
	...
	public function checkLogin(){
		...
		//echo $password=encode5t(isset($_REQUEST['password'])?$_REQUEST['password']:'');
		 $result = $this->Att_services_zone_model->checkLogin();
	}
	
	public function register_org(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->register_org();
	}
	
	public function getAllDesg($orgid){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getAllDesg($orgid);
	}
	
	public function getAllDept($orgid){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getAllDept($orgid);
	}
	public function DesignationMaster(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->DesignationMaster();
	}
	
	public function DepartmentMaster(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->DepartmentMaster();
	}
	
	public function shiftMaster(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->shiftMaster();
	}
	public function registerEmp(){
		
		header('Access-Control-Allow-Origin: *');
		try{
		$this->Att_services_zone_model->registerEmp();
		}catch(Exception $e){
			Trace($e->getMessage());
			echo $e->getMessage();
		}
		...
		header('Access-Control-Allow-Origin: *');
		try{
		$this->Att_services_zone_model->sendsms();
		}catch(Exception $e){
			Trace($e->getMessage());
			echo $e->getMessage();
		}
	...
	public function updateEmp(){
		...
		header('Access-Control-Allow-Origin: *');
		try{
		$this->Att_services_zone_model->updateEmp();
		}catch(Exception $e){
			Trace($e->getMessage());
			echo $e->getMessage();
		}
	...
	public function checkOrganization(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->checkOrganization();
	}
	...
	public function saveImage(){
		...
		$this->Att_services_zone_model->saveImage();  
	//	if($orgid==10)
			//$this->Att_services_zone_model->test();  
	//	else
	//		$this->Att_services_zone_model->saveImage();  
	}
	public function saveVisit(){
		header('Access-Control-Allow-Origin: *');
		$orgid   = isset($_REQUEST['refid']) ? $_REQUEST['refid'] : 0;
		$this->Att_services_zone_model->saveVisit();  
	}
	public function saveVisitOut(){
		header('Access-Control-Allow-Origin: *');
		$orgid   = isset($_REQUEST['refid']) ? $_REQUEST['refid'] : 0;
		$this->Att_services_zone_model->saveVisitOut();  
	}
	public function saveImageFromChrome(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->saveImageFromChrome();  
	}
	public	function getUserPermission(){
		try{	
			$perarray = $this->Att_services_zone_model->getUserPermission();
	...
	public function getInfo(){
		...
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getInfo();  
	}
	public function getPunchInfo(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getPunchInfo();  
	}
	public function getPunchedLocations(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getPunchedLocations();  
	}
	public function getHistory(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getHistory();  
	}
	public function getSlider(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getSlider();  
	}
	public function getAttendanceMobile(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getAttendanceMobile();  
	}
	public function getIndivisualReportData(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getIndivisualReportData();  
	}
	public function getLateComings(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getLateComings();  
	}
	public function getEarlyLeavings(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getEarlyLeavings();  
	}
	public function getBreakInfo(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getBreakInfo();  
	}
	public function timeBreak(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->timeBreak();  
	}
	public function changePassword(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->changePassword();  
	}
	public function getProfile(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getProfile();  
	}
	public function updateProfile(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->updateProfile();  
	}
	 public function updateProfilePhoto(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->updateProfilePhoto();  
	}
	public function resend_verification_mail(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->resend_verification_mail();  
	}
	public function resetPasswordLink(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->resetPasswordLink();  
	}
	public function HastaLaVistaUbi(){
		...
		$res=$this->Att_services_zone_model->checkLinkValidity($uid,$orgid,$counter);
	if($res){
		$data['uid']=$uid;
		$data['orgid']=$orgid;
...
//		$this->login_model->resetPasswordLinkPage($data);
		//$this->Att_services_zone_model->HastaLaVistaUbi();  
	}
	public function setPassword(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->setPassword();  
	}
	public function getUsersMobile(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getUsersMobile();  
	}
	public function getSuperviserSts(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getSuperviserSts(); 
	}
	public function addShift(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->addShift();  
	}
	public function addDesg(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->addDesg();  
	}
	public function updateDept(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->updateDept();
	}
	public function updateDesg(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->updateDesg();
	}
	public function updateShift(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->updateShift();
	}
	public function addDept(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->addDept();
	}
	public function addCheckin(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->addCheckin();
	}
	public function getTimeoffList(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getTimeoffList();  
	}
	public function getLeaveList(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getLeaveList();  
	}
	public function getAppVersion(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getAppVersion();  
	}
	public function UpdateStatus(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->UpdateStatus();  
	}
	public function checkMandUpdate(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->checkMandUpdate();  
	}
	...
	public function activateAccount(){
		header('Access-Control-Allow-Origin: *');
		if($this->Att_services_zone_model->activateAccount())
		{
	...
	public function activateOrg(){
		header('Access-Control-Allow-Origin: *');
		if($this->Att_services_zone_model->activateOrg())
		{
	...
	{
		 header('Access-Control-Allow-Origin: *');
		 $response = $this->Att_services_zone_model->getModules();
		 echo json_encode($response);
	}
	public function punchLocation(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->punchLocation();  
	}
	public function skipPunch(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->skipPunch();  
	}
	public function fetchTimeOffList(){
		header('Access-Control-Allow-Origin: *');
		try{
		$this->Att_services_zone_model->fetchTimeOffList();  
		}catch(Exception $e){echo 'Exceptipn occured';}
	}
/* 	public function reqForTimeOff(){
		header('Access-Control-Allow-Origin: *');
		try{
		$this->Att_services_zone_model->reqForTimeOff();  
		}catch(Exception $e){echo 'Exceptipn occured';}
	} */
	public function reqForTimeOff(){
		header('Access-Control-Allow-Origin: *');
		try{		
		$deptarray = $this->Att_services_zone_model->Createtimeoff();  
	...
	function changetimeoffsts(){	
		header('Access-Control-Allow-Origin: *');
		try{		
		$deptarray = $this->Att_services_zone_model->UpdateTimeoffSts();
		echo json_encode($deptarray);			
	...
	public function saveAllDesgPermission(){
		header('Access-Control-Allow-Origin: *');
		try{
		$perarray = $this->Att_services_zone_model->updatePermission();
		echo json_encode($perarray);
		}catch(Exception $e){echo 'Exceptipn occured';}
	}
	...
	public	function getAllDesgPermission(){
		try{	
			$perarray = $this->Att_services_zone_model->getAllDesgPermission();
			echo json_encode($perarray);
		}catch(Exception $e){
			echo $e->getMessage();
		}		
	...
	public function getEmpHistoryOf30(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getEmpHistoryOf30();  
	}
	public function getEmployeesList(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getEmployeesList();  
	}
	public function getAttendances_new(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getAttendances_new();  
	}
	public function getCDateAttendances_new(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getCDateAttendances_new();  
	}
	public function getCDateAttnDeptWise_new(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getCDateAttnDeptWise_new();  
	}
	public function getEmpdataDepartmentWise(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getEmpdataDepartmentWise();  
	}
	public function getCDateAttnDesgWise_new(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getCDateAttnDesgWise_new();  
	}
	
	public function getChartDataToday(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getChartDataToday();  
	}
	public function getAttendances_yes(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getAttendances_yes();  
	}
	public function getChartDataYes(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getChartDataYes();  
	}
	public function getChartDataCDate(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getChartDataCDate();  
	}
	public function getChartDataLast_7(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getChartDataLast_7();  
	}
	public function getChartDataLast_30(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getChartDataLast_30();  
	}
	public function getAttnDataLast(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getAttnDataLast();  
	}
	...
	//		echo getTimeZone(96);
	//		$res=0;
	//		$res=$this->Att_services_zone_model->mailtest();  
	...
	public function CreateBulkAtt(){
		header('Access-Control-Allow-Origin: *');
		try{
		$perarray = $this->Att_services_zone_model->CreateBulkAtt();
		echo json_encode($perarray);
		}catch(Exception $e){echo 'Exceptipn occured';}
	}
	...
	public function getDeptEmp(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getDeptEmp();  
	}
    ...
    header('Access-Control-Allow-Origin: *');
		...
		$this->Att_services_zone_model->backgroundLocationService();
    ...
    header('Access-Control-Allow-Origin: *');
		...
		$this->Att_services_zone_model->getEmplolyeeTrackTime();
	...
	public function getClientsDDList(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getClientsDDList();
	}
	
	public function saveFlexi(){
		header('Access-Control-Allow-Origin: *');
		$orgid   = isset($_REQUEST['refid']) ? $_REQUEST['refid'] : 0;
		$this->Att_services_zone_model->saveFlexi();  
	}
	public function saveFlexiOut(){
		header('Access-Control-Allow-Origin: *');
		$orgid   = isset($_REQUEST['refid']) ? $_REQUEST['refid'] : 0;
		$this->Att_services_zone_model->saveFlexiOut();  
	}
	public function getFlexiInfo(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getFlexiInfo();  
	}public function getAttendanceesFlexi(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getAttendanceesFlexi();  
	}
	public function getFlexiInfoReport(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getFlexiInfoReport();  
	}
	public function getOutsidegeoReport(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->getOutsidegeoReport();  
	}
...
public function saveOfflineData(){
	...
	header('Access-Control-Allow-Origin: *');
	$this->Att_services_zone_model->saveOfflineData();  
}
public function saveOfflineVisits(){
	
	header('Access-Control-Allow-Origin: *');
	$this->Att_services_zone_model->saveOfflineVisits();  
}

public function getNotifications(){
	header('Access-Control-Allow-Origin: *');
	$this->Att_services_zone_model->getNotifications();
}

public function addHoliday(){
		header('Access-Control-Allow-Origin: *');
		$this->Att_services_zone_model->addHoliday();  
	}

public function getAllHoliday(){
		header('Access-Control-Allow-Origin: *');
		$orgid   = isset($_REQUEST['orgid']) ? $_REQUEST['orgid'] : 0;
		$this->Att_services_zone_model->getAllHoliday($orgid);  
	}
	public function saveOfflineQRData(){
		header('Access-Control-Allow-Origin: *');
		
		$this->Att_services_zone_model->saveOfflineQRData();  
	}

	public function checkNextWorkingDayCode(){
		header('Access-Control-Allow-Origin: *');
		
		$this->Att_services_zone_model->checkNextWorkingDayCode();  
	}
	
	public function syncAshTechData(){
		header('Access-Control-Allow-Origin: *');
		
		$this->Att_services_zone_model->syncAshTechData();  
	}
}	
?>
