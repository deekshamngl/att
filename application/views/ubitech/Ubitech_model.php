...
*/
	...
	/*<i class="material-icons delete2" data-toggle="modal" data-target="#delLead" data-aid="'.$row->id.'" data-image="'.$row->filename.'" title="Delete" style="cursor:pointer">close</i>	*/
		...
		public function getExtrauserOrg()
		{
			...
			$query = $this->db->query("SELECT NoOfEmp,Name,Email,Id,Country,PhoneNumber,Address,Organization.rating as rating,Organization.leadowner_id, (select count(Id) from UserMaster where UserMaster.OrganizationId = Organization.Id ) as users,(select licence_ubiattendance.status from licence_ubiattendance where licence_ubiattendance.OrganizationId=Organization.Id limit 1) as sstts FROM Organization  $q Having users > NoOfEmp ");
		...
		public function editOrganizationData($id){
				...
				$query = $this->db->query("select due_amount,Addon_BulkAttn,Addon_LocationTracking,Addon_VisitPunch,Addon_GeoFence,Addon_Payroll,Addon_TimeOff,Addon_flexi_shif,Addon_offline_mode ,Addon_Edit, Addon_Delete from licence_ubiattendance where OrganizationId = $id");
	...
	public function getstorepackage1(){
		  ...
		  if($phone!=$phone1)
		   ...
		   if($flage==1)
			...
			$query1 = $this->db->query("Update UserMaster SET Username = '".encode5t($email)."' where Username = '".encode5t($email1)."'");	
             ...
             $query = $this->db->query("update Organization set Name='$org_name',PhoneNumber=$phone,Email='$email',Country=$county,Address='$address',customize_org='$orgtype',TimeZone='$TimeZone' ,leadowner_id='$lead',rating='$rating' where Id = $id");
		...
		public function trial(){
			...
			if($this->db->affected_rows()>0)
			{
				$sql = "update Organization set NoOfEmp='$limit',cleaned_up=0,delete_sts=0 where Id=$org_id";
				$query = $this->db->query($sql);
		...
		public function getPaymentHistoryData($orgid)
		{
			...
			$query = $this->db->query("SELECT `id`,OrganizationId, `txnid`, `indivisual_name`,`name`, `email`, `payment_amount`, `createDate`, `tax`, `discount`, `currency`,action,remark,payment_status FROM `payments_invoice` ".$cond." order by Id desc");
			$d=array();
			$res=array();
			 foreach ($query->result() as $row)
			{
