  ...
  <body onload="submitPayuForm()">
    ...
    <div class="main-panel">
      ...
      <div class="content">
            ...
            <div class="col-md-12" >
              <div class="card valign">
                <div class="card-header" data-background-color="orange">
                  <div class="nav-tabs-navigation">
                    <div class="nav-tabs-wrapper">
                      <h6 class="nav-tabs-title">Upgrade Plan<a onClick="location.reload();" style="margin:0px; padding: 5px 15px; background-color:#45a149;" class="pull-right btn">Reset</a></h6>
                ...
                <div class="card-content table-responsive">
				 ...
				 <div id="operation" class="col-sm-4 col-md-4 col-xs-4">
							...
							<div class="panel panel-default">
								...
								<table class="tab1">
									  <tr>
										...
										<th><center>Bulk Attendance</center></th>
										<th><center>Location Tracing</center></th>
										<th><center>Visit Punch</center></th>
										<th><center>Geo Fence</center></th>
										<th><center>Payroll</center></th>
	   ...
	   <!-------Addons tab start--------->
                <div class="accordion_main">
                    <div class="accordion_head">Addons Details<button id="edit6" type="button" class="btn editbutton"><i  class="fa fa-pencil " style="font-size:12px;margin-top:0px;top:0px;"></i> Edit</button></div>
				  ...
				  <table class="table table-responsive" style="width:80%" id="ordertable">
                    ...
                    <td width="20%" align="right"><input type='checkbox' name="bulk_attcheck" id="bulk_attcheck" <?php if($myplan['Addon_BulkAttnP']==1) echo "checked "; ?>  /></td>
                    </tr>
                    ...
                    <td width="20%" align="right"><input type='checkbox'  name="location_tracecheck" id="location_tracecheck" <?php if($myplan['Addon_LocationTrackingP']==1) echo "checked "; ?>  /></td>
                    </tr>
					
					<tr>
                    <td width="50%" align="left" id="visit_perprice">Visit Punch</td>
                    <td width="25%" align="right"><span id="visit_punchprice"></span></td>
                    <td width="20%" align="right"><input type='checkbox' name="visit_punchcheck" id="visit_punchcheck" <?php if($myplan['Addon_VisitPunchP']==1) echo "checked "; ?>  /></td>
                    </tr>
                   
				    <tr>
                    <td width="50%" align="left" id="gfence_perprice">Geo Fence</td>
                    <td width="25%" align="right"><span id="geo_fenceprice"></span></td>
                    <td width="20%" align="right"><input type='checkbox' name="geo_fencecheck" id="geo_fencecheck" <?php if($myplan['Addon_GeoFenceP']==1) echo "checked "; ?> /></td>
                    </tr>
					
				    <tr>
                    <td width="50%" align="left" id="payroll_perprice">Payroll</td>
                    <td width="25%" align="right"><span id="payroll_price"></span></td>
                    <td width="20%" align="right"><input type='checkbox' name="payroll_check" id="payroll_check"  <?php if($myplan['Addon_PayrollP']==1) echo "checked "; ?> /></td>
                    </tr>
					
                    <tr>
                    <td width="50%" align="left" id="timeoff_perprice">Time Off</td>
                    <td width="25%" align="right"><span id="timeoff_price"></span></td>
                    <td width="20%" align="right"><input type='checkbox' name="timeoff_check" id="timeoff_check" <?php if($myplan['Addon_TimeOffP']==1) echo "checked "; ?>  /></td>
                    </tr>
    ...
    $("#continue2").click(function(){ 
		...
		if(action=="UO"){
		   ...
		   switch (true) {
			 ...
			 $("#bulk_attprice").text((no_of_users *Number($('#'+bulkattendance+tempvar ).text())*remaining_days/30.45).toFixed(2));
			  <?php } 
			   else{
			  ?>
			   $("#bulk_attprice").text((x *Number($('#'+bulkattendance+tempvar ).text())*remaining_days/30.45).toFixed(2));
			   <?php } ?>
			   
			 <?php if($myplan['Addon_LocationTrackingP']==1) { ?>
			 $("#location_traceprice").text((no_of_users *Number($('#'+locationtracing+tempvar ).text())*remaining_days/30.45).toFixed(2));
			    <?php } 
			   else{
			  ?>
			  $("#location_traceprice").text((x *Number($('#'+locationtracing+tempvar ).text())*remaining_days/30.45).toFixed(2));
			   <?php } ?>
			 
			  <?php if($myplan['Addon_VisitPunchP']==1) { ?>
			  $("#visit_punchprice").text((no_of_users *Number($('#'+visitpunch+tempvar ).text())*remaining_days/30.45).toFixed(2));
			     <?php } 
			   else{
			  ?>
			  $("#visit_punchprice").text((x *Number($('#'+visitpunch+tempvar ).text())*remaining_days/30.45).toFixed(2));
			   <?php } ?>
			  
			   <?php if($myplan['Addon_GeoFenceP']==1) { ?>
			  $("#geo_fenceprice").text((no_of_users *Number($('#'+geofence+tempvar ).text())*remaining_days/30.45).toFixed(2));
			   <?php } 
			   else{
			  ?>
			   $("#geo_fenceprice").text((x *Number($('#'+geofence+tempvar ).text())*remaining_days/30.45).toFixed(2));
			   <?php } ?>
			  
			  <?php if($myplan['Addon_PayrollP']==1) { ?>
			  $("#payroll_price").text((no_of_users *Number($('#'+payroll+tempvar ).text())*remaining_days/30.45).toFixed(2));
			   <?php } 
			   else{
			  ?>
			  $("#payroll_price").text((x *Number($('#'+payroll+tempvar ).text())*remaining_days/30.45).toFixed(2));
			   <?php } ?>
			  
			  <?php if($myplan['Addon_TimeOffP']==1) { ?> 
			  $("#timeoff_price").text((no_of_users *Number($('#'+timeoff+tempvar ).text())*remaining_days/30.45).toFixed(2));
			   <?php } 
			   
			   else{
			  ?>
			  $("#timeoff_price").text((x *Number($('#'+timeoff+tempvar ).text())*remaining_days/30.45).toFixed(2));
	<?php } ?>
			 ...
			 if(plan=='YEARLY'){
				...
				amount = Math.round(no_of_users * (($('#rate').text())/30.45) * remaining_days);
				$('#rateindays').text(($('#rate').text())+'/30.45');
			}
		...
		//	alert('update the subs period only');
		   ...
		   switch (true) {
			  ...
			  $('#timeoff_perprice').text("Time Off @ "+tempcurrency+". "+$('#'+timeoff+tempvar ).text()+"/month");
			    ...
			    // calculate price 
			  <?php if($myplan['Addon_BulkAttnP']==1) { ?>
			 $("#bulk_attprice").text((x *Number($('#'+bulkattendance+tempvar ).text())*extendday/30.45).toFixed(2));
			  <?php } 
			   else{
			  ?>
			   $("#bulk_attprice").text((x *Number($('#'+bulkattendance+tempvar ).text())*(remaining_days+extendday)/30.45).toFixed(2));
			   <?php } ?>
			   
			 <?php if($myplan['Addon_LocationTrackingP']==1) { ?>
			 $("#location_traceprice").text((x *Number($('#'+locationtracing+tempvar ).text())*extendday/30.45).toFixed(2));
			    <?php } 
			   else{
			  ?>
			  $("#location_traceprice").text((x *Number($('#'+locationtracing+tempvar ).text())*(remaining_days+extendday)/30.45).toFixed(2));
			   <?php } ?>
			 
			  <?php if($myplan['Addon_VisitPunchP']==1) { ?>
			  $("#visit_punchprice").text((x *Number($('#'+visitpunch+tempvar ).text())*extendday/30.45).toFixed(2));
			     <?php } 
			   else{
			  ?>
			  $("#visit_punchprice").text((x *Number($('#'+visitpunch+tempvar ).text())*(remaining_days+extendday)/30.45).toFixed(2));
			   <?php } ?>
			  
			   <?php if($myplan['Addon_GeoFenceP']==1) { ?>
			  $("#geo_fenceprice").text((x *Number($('#'+geofence+tempvar ).text())*extendday/30.45).toFixed(2));
			   <?php } 
			   else{
			  ?>
			   $("#geo_fenceprice").text((x *Number($('#'+geofence+tempvar ).text())*(remaining_days+extendday)/30.45).toFixed(2));
			   <?php } ?>
			  
			  <?php if($myplan['Addon_PayrollP']==1) { ?>
			  $("#payroll_price").text((x *Number($('#'+payroll+tempvar ).text())*extendday/30.45).toFixed(2));
			   <?php } 
			   else{
			  ?>
			  $("#payroll_price").text((x *Number($('#'+payroll+tempvar ).text())*(remaining_days+extendday)/30.45).toFixed(2));
			   <?php } ?>
			  
			  <?php if($myplan['Addon_TimeOffP']==1) { ?> 
			  $("#timeoff_price").text((no_of_users *Number($('#'+timeoff+tempvar ).text())*extendday/30.45).toFixed(2));
			   <?php } 
			   else{
			  ?>
			  $("#timeoff_price").text((x *Number($('#'+timeoff+tempvar ).text())*(remaining_days+extendday)/30.45).toFixed(2));
			   <?php } ?>
